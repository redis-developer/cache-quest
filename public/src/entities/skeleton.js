...
import { spawnScale } from "../systems/drops.js";

export function spawnSkeleton(enemy) {
  return [
    k.sprite("skeleton", { anim: "idle" }),
    ...
    enemy.type,
    {
      ...
      setEvents() {
        this.onHurt(() => {
          // console.log(`skeleton ${this.id} hurt! Current HP: ${this.hp}`);
          if (this.hp <= 0) this.enterState("death");
          blinkEffect(this);
        });

        this.onCollide("playerSword", (sword) => {
          // console.log("skeleton hit by sword!");
      ...
      setBehavior() {
        ...
        this.onStateEnter("idle", async () => {
          // console.log(
          //   `skeleton ${this.id} entered idle, player distance: ${this.pos.dist(player.pos)}`
          // );
        ...
        this.onStateEnter("patrol-right", async () => {
          // console.log(
          //   `skeleton ${this.id} entered patrol-right, player distance: ${this.pos.dist(player.pos)}`
          // );
          this.flipX = false;
        ...
        this.onStateEnter("patrol-left", async () => {
          // console.log(
          //   `skeleton ${this.id} entered patrol-left, player distance: ${this.pos.dist(player.pos)}`
          // );
          this.flipX = true;
          this.offset = k.vec2(-20, -10);
          if (!this.isAttacking) playAnimIfNotPlaying(this, "run");
          await this.wait(3);
        ...
        this.onStateEnter("alert", () => {
          // console.log(
          //   `skeleton ${this.id} entered alert, player distance: ${this.pos.dist(player.pos)}`
          // );
          if (!this.isAttacking) playAnimIfNotPlaying(this, "run");
        ...
        this.onStateEnter("attack", async () => {
          // console.log("skeleton attacking");
          ...
          await this.wait(0.3);
          enAtk = this.add(
            enemyAttackHitbox(k.vec2(this.flipX ? -35 : 35, -20), k.vec2(70, 90), this.attackPower),
          );
        });

        this.onStateEnter("death", () => {
          // console.log(`skeleton ${this.id} died!`);
          this.isAlive = false;
          playerState.set(
            "enemiesKilled",
            "skeleton",
            playerState.get().enemiesKilled.skeleton + 1,
          );

          k.add(spawnScale(this.pos)).init();
          this.destroy();
        });
...
export function spawnSkeletonSurvivor(enemy, target, score) {
  return [
    k.sprite("skeletonSurvivor", { anim: "run" }),
